...
async function isEmpty(): Promise<boolean> {
  const checks = await Promise.all([
    nodeDevDb.execute(sql`SELECT EXISTS(SELECT 1 FROM ${users}) AS v`),
    nodeDevDb.execute(sql`SELECT EXISTS(SELECT 1 FROM ${customers}) AS v`),
    nodeDevDb.execute(sql`SELECT EXISTS(SELECT 1 FROM ${invoices}) AS v`),
    nodeDevDb.execute(sql`SELECT EXISTS(SELECT 1 FROM ${revenues}) AS v`),
    nodeDevDb.execute(
