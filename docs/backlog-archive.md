...
The long tail of [`BACKLOG.md`](../BACKLOG.md)'s `## Done` log. Newest first, same
format, same meaning — these are finished, not abandoned.

**Why this file exists.** `BACKLOG.md` is opened by every session and sits in the
editor all day, so it is kept small: `## Open` plus only the **10 most recent**
completed items. Everything older rolls down here. The split is by count rather
than by date so the live file's size does not swing with how busy a month was.

**What to do with it.** Grep, don't read. These entries exist so a future session
can find the verdict on something that was already settled — a "don't redo this",
a dead end and why it was a dead end, the reason a hold or an override is still in
place. Deleting them would technically leave them in git history, but history is
not greppable in an editor, and several `project_*` memory files point at these
lines by name.

**Fuller detail lives elsewhere.** For anything here that needs more than a
paragraph, the durable write-up is in the matching `project_*` memory file, not in
this archive.
