...
> The question this answers: _"From the moment someone logs in to the moment their
> session is gone, what states can it be in, and what moves it between them?"_ The
> [login diagram](auth-login-flow.md) shows the _sequence_ of a single login; this
> shows the **state machine** that governs the session afterwards.

This is the picture (the _what_); the decision behind it — _why_ sessions are
...
(`exp`/`nbf`, with a ±5 s clock tolerance), so an _expired_ token is rejected at
...
There is **no sessions table**. The JWT _is_ the session — everything needed to
...
deletes _this_ browser's cookie; it doesn't reach out and kill a token copied
