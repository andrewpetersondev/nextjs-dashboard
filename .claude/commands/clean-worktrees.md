...
allowed-tools: Bash(git fetch:*), Bash(git worktree list:*), Bash(git worktree remove:*), Bash(git branch -vv:*), Bash(git branch --list:*), Bash(git status:*), Bash(git -C:*), Bash(git rev-parse:*), Bash(git rev-list:*), Bash(git merge-base:*), Bash(git log:*)
---
...
3. **Classify every candidate branch** (all branches except the off-limits set). A branch is **SAFE** only if
   `git rev-list --count origin/main..<branch>` is `0` (it has no commits that aren't already in main). This
   repo's local-first flow merges with real merge commits, so merged lanes always reach 0.

   A branch with a nonzero count is **NOT safe**, even if its upstream is `[gone]` — it may be abandoned
   work or a squash-merged lane from the pre-2026-06 PR era. List it under "needs your call" and never
   delete it.
...
6. **Emit the branch-delete block.** For every SAFE branch that no longer has a worktree, output a single
   fenced ```bash block the user can paste, one `git branch -d <branch>` per line. `-d` should succeed for
   every branch step 3 proved contained; if one refuses, re-verify that branch instead of reaching for `-D`.
