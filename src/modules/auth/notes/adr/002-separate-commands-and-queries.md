...
As the Auth module grows, mixing business logic that modifies state (Commands) with logic that only retrieves data (
Queries) can lead to bloated service classes and complex side effects. We need a clear separation to improve
...
- **Commands**: Encapsulated in "Use Cases" (e.g., `LoginUseCase`, `SignupUseCase`). These represent actions that change
  the state of the system or perform significant side effects (like setting cookies).
- **Workflows**: Used to orchestrate multiple Commands or complex multi-step processes (e.g., `LoginWorkflow`
  orchestrates `LoginUseCase` and `EstablishSessionUseCase`).
