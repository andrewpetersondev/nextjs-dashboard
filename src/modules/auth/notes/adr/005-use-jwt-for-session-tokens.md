...
## Context
We need a stateless way to manage user sessions that works well with Next.js Server Actions and Middleware. Using server-side session stores (like Redis) adds infrastructure complexity and latency for session validation on every request.
...
- **Rotation**: Sessions should support rotation (refreshing the expiration) to keep users logged in during active use while maintaining security.
...
- **Performance**: Token verification is a local cryptographic operation, avoiding network round-trips to a database/cache for every request.
- **Compatibility**: Works naturally with Next.js edge runtime and middleware.

### Negative
- **Revocation**: Difficult to revoke a single token before it expires without maintaining a blacklist (which reintroduces state).
