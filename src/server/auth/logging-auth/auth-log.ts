...
/**
 * Create a scoped auth logger for multiple related log calls.
 * Useful when you need to log multiple events in the same operation.
 *
 * @example
 * const authLogger = createAuthScopedLogger(requestId);
 * authLogger.info("Login started", AuthLog.action.login.start());
 * authLogger.error("Login failed", AuthLog.action.login.error(error));
 */
export function createAuthScopedLogger(requestId?: string): {
  debug: (
    message: string,
    payload: AuthLogPayload,
    additionalData?: Record<string, unknown>,
  ...
  return {
    debug: (message, payload, additionalData) =>
      logAuth("debug", message, payload, { additionalData, requestId }),
    error: (message, payload, additionalData) =>
      logAuth("error", message, payload, { additionalData, requestId }),
    info: (message, payload, additionalData) =>
      logAuth("info", message, payload, { additionalData, requestId }),
    warn: (message, payload, additionalData) =>
      logAuth("warn", message, payload, { additionalData, requestId }),
  };
}
