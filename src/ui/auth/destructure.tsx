...
// Email input field
function LoginEmailField({ error }: { error?: string[] }) {
	return (
		<>
			<div>
...
function LoginPasswordField({ error }: { error?: string[] }) {
	return (
		<>
			<div>
				<label
			...
			{error && (
				<div className="text-text-error">
					...
					<ul>
						{error.map((err) => (
							<li key={err} data-cy="login-password-errors">
								- {err}
							</li>
						))}
					</ul>
			...
			<div className="mt-10 sm:mx-auto sm:w-full sm:max-w-[480px]">
				<div className="bg-bg-secondary px-6 py-12 shadow-sm sm:rounded-lg sm:px-12">
					<form action={action} className="space-y-6">
						<LoginEmailField error={state?.errors?.email} />
						<LoginPasswordField error={state?.errors?.password} />
