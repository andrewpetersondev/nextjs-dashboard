...
async function ensureResetOrEmpty(): Promise<boolean> {
...
async function buildUserSeed(): Promise<
  ReadonlyArray<{
...
async function insertRevenues(tx: Tx): Promise<void> {
...
async function insertCustomers(tx: Tx): Promise<void> {
...
function buildRandomInvoiceRows(
...
async function insertDemoCounters(tx: Tx): Promise<void> {
...
async function aggregateRevenues(tx: Tx): Promise<void> {
...
export async function mainCypTestSeed(): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.insert(schema.users).values(userSeed as readonly any[]);
